...
bool stm32h7_probe(target *t)
{
	...
	if (idcode == ID_STM32H74x) {
		...
		target_add_commands(t, stm32h7_cmd_list, "stm32h7");
		return true;
	}
	return false;
}

static bool stm3h7_attach(target *t)
{
	if (!cortexm_attach(t))
		return false;
	...
	/* size per bank! */
	uint32_t banksizesize;
	anksizesize = (target_mem_read32(t, flashsize_base) & 0xffff) << 9;
	stm32h7_add_flash(t, 0x8000000           , banksize, FLASH_SECTOR_SIZE);
	stm32h7_add_flash(t, 0x8000000 + banksize, banksize, FLASH_SECTOR_SIZE);
							   ...
							   size_t len)
{
	...
	enum mem_access psize = ((struct stm32h7_flash *)f)->psize;
	while (end_sector >= start_sector) {
		uint32_t cr = ((2 - psize) * FLASH_CR_PSIZE16) | FLASH_CR_SER |
                               ...
                               const void *src, size_t len)
{
	target *t = f->t;
	struct stm32h7_flash *sf = (struct stm32h7_flash *)f;
	enum mem_access psize = sf->psize;
	target_mem_write32(t, sf->regbase + FLASH_CR,
					   ((2 - psize) * FLASH_CR_PSIZE16) | FLASH_CR_PG);
	/* Use RM0433 Rev 3. 3.3.7 "Optimal block write sequence"
	 * We rely on system stalling with write command queue
	 * and the write buffer full.
	 */
...
static bool stm32h7_cmd_erase(target *t, int bank_mask)
{
	...
	tc_printf(t, "\n");
	if (!result) {
		DEBUG("sr %" PRIx32 " cr %" PRIx32 ", sr2 %" PRIx32
			  ",cr2 %" PRIx32 "\n",
